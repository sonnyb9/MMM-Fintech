...
class SnapTradeRestClient {
  ...
  async request({ method, path, queryParams, body }) {
    ...
    const signedQuery = buildQuery(authQueryParams);

    const signedContent =
      body && Object.keys(body).length > 0 ? body : otherQueryParams;
    ...
    const url = `${this.baseUrl}${this.apiVersionPath}${path}?${urlQuery}`;
